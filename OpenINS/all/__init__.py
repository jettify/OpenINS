...
__all__ = []
path = os.path.split(pkgutil.get_loader('OpenINS').filename)[0]
for loader, modname, ispkg in pkgutil.walk_packages([path]):
    if modname.startswith('OpenINS') \
            and not modname.startswith('OpenINS.tests') \
            and not modname.startswith('OpenINS.all'):
